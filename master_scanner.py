...
def get_index_options_ideas():
    ...
    for name, symbol, step in [('NIFTY 50', '^NSEI', 50), ('BANK NIFTY', '^NSEBANK', 100)]:
        try:
            df = yf.download(symbol, period="1mo", interval="1d", progress=False, threads=False) # Threads disabled here to prevent hang
...
def run():
    ...
    for i in range(0, len(tickers), chunk_size):
        ...
        try:
            ...
            for ticker in chunk:
                try:
                    ...
                    # 1. Strict Scan (Requires EMA + Base Score >= 2)
                    if passes_ema and base_score >= 2:
                        rec_strict = record.copy()
                        rec_strict['Score'] = base_score
                        res_strict.append(rec_strict)
                        
                        # 3. Budget Scan (Requires Strict Pass + Price < 500)
                        if close_p < 500:
                            res_budget.append(rec_strict)

                    # 2. Aggressive Scan (Ignores EMA + Score Bonus + Base Score >= 2 logic)
        ...
        time.sleep(1) # Breathe between chunks to keep Yahoo happy
